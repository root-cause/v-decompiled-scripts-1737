...
void __EntryFunction__()
{
	...
	if (SCRIPT::_GET_NUMBER_OF_REFERENCES_OF_SCRIPT_WITH_NAME_HASH(SCRIPT::GET_HASH_OF_THIS_SCRIPT_NAME()) > 1 && !func_130(uLocal_58, 4194304))
	{
		if (iLocal_69 != 263)
		{
	...
	if (!ENTITY::IS_ENTITY_DEAD(PLAYER::PLAYER_PED_ID(), 0))
	{
		...
		if (ENTITY::DOES_ENTITY_EXIST(iLocal_255))
		{
			...
			if (fVar7 > 0.8f)
			{
				...
				if (iLocal_82 == 12)
				{
					TASK::TASK_FOLLOW_NAV_MESH_TO_COORD(0, vVar5, 1f, 20000, 1048576000, 0, func_63(vVar5, ENTITY::GET_ENTITY_COORDS(iLocal_255, false)));
				}
				else
				{
					TASK::TASK_GO_STRAIGHT_TO_COORD(0, vVar5, 1f, -1, func_63(vVar5, ENTITY::GET_ENTITY_COORDS(iLocal_255, false)), 1056964608);
				}
			}
			else if (fVar7 > 0.15f)
			{
				TASK::TASK_ACHIEVE_HEADING(0, func_63(ENTITY::GET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), true), ENTITY::GET_ENTITY_COORDS(iLocal_255, false)), 0);
			}
